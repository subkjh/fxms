...
public class CheckUtil {
	...
	public static void checkIp(String ipAddress) throws Exception {
		if (isIpAddress(ipAddress) == false)
			throw new Exception(Lang.get("입력 내용이 IP주소가 아닙니다.") + " : " + ipAddress);
	}

	public static void checkIp(String ipAddress, String name) throws Exception {
		if (isIpAddress(ipAddress) == false)
			throw new Exception(Lang.get("IP주소가 아닙니다.") + " " + name + ", " + ipAddress);
	}
	...
	public static void checkPassword(String s, int maxLen) throws Exception {
		...
		if (s.length() > maxLen) {
			throw new Exception(Lang.get("암호는 다음 길이를 초과할 수 없습니다.") + "(" + maxLen + ")");
		}

		// 2013.06.18 by subkjh
		if (isAllowSimplePassword) {
			if (s == null || s.trim().length() == 0) {
				throw new Exception(Lang.get("암호는 최소 1자리 이상이여야합니다."));
			}
			return;
		}

		String msg = Lang.get("암호는 다음 조건을 충족해야 합니다.") + "\n" //
				+ Lang.get("최소 8자리 이상이고 영문, 숫자, 특수문자가 각각 1자 이상 포함되어야 합니다") + "\n" //
				+ Lang.get("자판배열에서 4자리 이상이 연속으로 구성될 수 없습니다.") + "\n" //
				+ Lang.get("같은 문자가 연속해서 3번이상 나올수 없습니다.");

...
public class AckAlarmBox extends FxGridPane implements DxListener<UiAlarm>, DxNode {

	public AckAlarmBox() {

		super("Acked Alarms", new GridCfg(5, 70, 20));
	...
	public void onData(Action action, final UiAlarm data) {
		...
		// System.out.println("ack " + action + " " + id);
		...
		if (action == Action.update) {
			...
			btn.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					FxStage.showDialog(AckAlarmBox.this, new ShowAlarmDetailPane(data), "경보상세보기");
				}
			});

			btn.setUserData(data);
			btn.getStyleClass().add("alarm-level-" + data.getAlarmLevel() + "-acked");

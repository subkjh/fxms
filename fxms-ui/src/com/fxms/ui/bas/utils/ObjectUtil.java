...
public class ObjectUtil {
	...
	public static void toCopy(Object src, Object dst) {

		List<Field> dstFields = getFieldList(dst.getClass());
		List<Field> srcFields = getFieldList(src.getClass());

		for (Field dstFeild : dstFields) {
			for (Field srcField : srcFields) {
				if (dstFeild.getName().equals(srcField.getName())) {
					try {
						dstFeild.setAccessible(true);
						srcField.setAccessible(true);
						setField(dst, dstFeild, srcField.get(src));
					} catch (Exception e) {
						e.printStackTrace();
					}
					break;

...
public class QidPool {
	...
	private Map<String, ResultBean> resultMap = new HashMap<String, ResultBean>();
	...
	public QidPool(String database, InputStream inputStream) throws Exception {
		...
		SqlXmlParser parser = new SqlXmlParser(constMap, resultMap, database);
	...
	public boolean exist(String filename) {
		...
		return SqlPointer.class.getResourceAsStream(urlname) != null;
	}
	...
	private void setRet(SqlXmlParser parser) throws Exception {
		Map<String, ResultBean> retMap = parser.getResultMap();
		List<SqlBean> qidList = parser.getQidList();
		for (SqlBean qid : qidList) {
			add(qid);
		}

		for (ResultBean ret : retMap.values()) {
			ResultBean retOld = resultMap.get(ret.getResultId());
			if (retOld.equals(ret) == false) {
				Logger.logger.info("Check Same ReulstID different Attr", ret.getResultId());
			}
			resultMap.put(ret.getResultId(), ret);
		}

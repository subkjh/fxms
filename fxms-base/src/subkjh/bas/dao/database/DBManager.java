...
import subkjh.bas.dao.exception.DBTypeNotFoundException;
import subkjh.bas.dao.model.SqlBean;
import subkjh.bas.dao.utils.MakeJavaSource;
...
public class DBManager {
	...
	public static void main(String[] args) throws Exception {
		DBManager.getMgr().init();
		System.out.println(DBManager.getMgr().getDebug());
	}
	...
	private void initSqlPoolCommon() {

		File file = new File(BasCfg.getHomeDeployConfSql() + File.separator + "queries.xml");

		Logger.logger.info("FILE-PATH(" + file.getPath() + ") " + (file.exists() ? "EXIST" : "NOT FOUND"));

		if (file.exists()) {
			try {
				FileInputStream is = new FileInputStream(file);
				sqlPoolCommon = new QidPool(null, is);
				is.close();
			} catch (Exception e1) {
			...
			try {
				InputStream inputStream = MakeJavaSource.class.getResourceAsStream("queries.xml");
				if (inputStream == null) {
					Logger.logger.error("FILE-NAME(" + MakeJavaSource.class.getPackage() + ".queries.xml) NOT FOUND");
				} else {
					sqlPoolCommon = new QidPool(null, inputStream);
				}
			} catch (Exception e1) {
				Logger.logger.error(e1);
			}

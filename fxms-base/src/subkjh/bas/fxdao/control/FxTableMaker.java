...
public class FxTableMaker {
	...
	public List<Table> makeTableList(Class<?> classOfMain) throws Exception {
		...
		// tableMap.put(classOfMain, tableList);
	...
	private Column getIndexColumn(Class<?> classOf, String columnName, Table table) throws Exception {
		...
		Field field;
		try {
			field = classOf.getDeclaredField(fieldName);
			column = getColumn(field);
			if (column != null) {
				return column;
			}
		} catch (NoSuchFieldException e) {
			return getIndexColumn(classOf, columnName, table);
	...
	private Table getTable(Class<?> classOfMain, Class<?> classOf) throws Exception {

...
public abstract class EventApi extends FxApi {
	...
	/** 경보등급 */
	public static final String PARA_FO_LEVEL = "foLevel";
	/** 비교 값 */
	...
	public Alarm check(Mo mo, String moInstance, AlarmCode alarmCode, String msg, Map<String, Object> para) {
		...
		if (para != null) {
			Object tmp = para.get(PARA_FO_LEVEL);
		...
		if (foLevel == null)
			foLevel = AoCode.AlarmLevel.getLevel(alarmCode.getAlarmLevel());
	 ...
	 * @return 관련된 경보 목록
	 */
	public List<Alarm> checkPsValue(Mo mo, String instance, String psCode, Number psPrevValue, Number psCurValue,
			long pollMsdate) {
		return checkPsValue(mo, instance, psCode, psPrevValue, psCurValue, pollMsdate, 0, 0, null, null);
	}

	public List<Alarm> checkPsValue(Mo mo, String instance, Enum<?> psCode, Number psPrevValue, Number psCurValue,
			long pollMsdate) {
	...
	public List<Alarm> checkPsValue(Mo mo, String moInstance, String psCode, Number psPrevValue, Number psCurValue,
		...
		try {
			...
			List<AlarmEvent> events = makeEventPs(mo, moInstance, psCode, psPrevValue, psCurValue, pollMsdate, dpPerfNo,
					dpValue);
	...
	public List<Alarm> checkPsValue(Mo mo, String instance, String psCode, Number valuePrev, Number valueCur,
	...
	public abstract OccurAlarmDbo doSelectAlarmHst(long alarmNo) throws Exception;
	 ...
	 * @return 경보조건번호
	 */
	public int getPerfThrNo(long moNo) {
	...
	public AlarmEvent makeEvent(Mo mo, String moInstance, int alcdNo) {
		...
		AlarmEvent event = makeEvent(mo.getMoNo(), moInstance, alarmCode.getAlcdNo());
		event.setAlarmLevel(alarmCode.getAlarmLevel());
	...
	public AlarmEvent makeEvent(Mo mo, String moInstance, int alcdNo, String msg, Number valueBase, Number psValuePrev,
		...
		// 경보등급이 정의되어 있지 않으면 경보코드 기본 메세지를 사용합니다.
		if (alarmLevel < 0)
			alarmLevel = alarmCode.getAlarmLevel();
	...
	private int getAlarmCfgNo(Mo mo) {
		...
		if (alarmCfgNo == 0) {
			if (mo.getUpperMoNo() > 0) {
				...
				if (moUpper != null) {
					alarmCfgNo = moUpper.getAlarmCfgNo();
				}
			}
		}
	 ...
	 * @return
	 */
	private AlarmEvent makeClearEvent(Mo mo, String instance, long alarmNo, int alcdNo, AoCode.ClearReason clearReason,
	 ...
	 * @throws Exception
	 */
	private AlarmEvent makeEvent(AlarmCfg thr, Mo mo, String instance, AlarmCode alarmCode, String psCode,
			Number valuePre, Number valueCur, long hstime, int dpPerfNo, double dpValue) throws Exception {
		...
		if (cfgMem == null) {
			...
			if (alarmOld != null) {
				return makeClearEvent(mo, instance, alarmOld.getAlarmNo(), alcdNo,
						AoCode.ClearReason.AlarmCfgMemNotFound, valueCur);
			}
		}
	 ...
	 * @return 만들어진 이벤트 메시지
	 */
	private String makeEventMsg(String orgMsg, Mo mo, String foInstance, Object baseValue, Object preValue,
	...
	private List<AlarmEvent> makeEventPs(Mo mo, String instance, String psCode, Number psPrevValue, Number psCurValue,
		...
		if (alcdNos == null || alcdNos.size() == 0) {
			return makeEventClearByPerf(psCode, mo, instance, psCurValue,
					AoCode.ClearReason.AlarmCodeNotFoundForPsCode);
		}

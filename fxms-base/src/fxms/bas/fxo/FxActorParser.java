...
public class FxActorParser {
	...
	public <T> T getActor(Class<T> classOfT, String name, String value) {
		...
		for (FxActor f : list) {
			if (classOfT.isInstance(f)) {
				...
				if (f instanceof FxActorImpl) {
					if (((FxActorImpl) f).matchAttr(name, value)) {
						return (T) f;
					}
				}
			}
		...
		for (Element node : nodeList) {
			...
			try {
				...
				if (obj instanceof FxActor) {
					...
					if (actor instanceof FxActorImpl) {
						((FxActorImpl) actor).setAllMatch("true".equalsIgnoreCase(allMatch));
						setCondition((FxActorImpl) actor, node);
						setParameters((FxActorImpl) actor, node);
						((FxActorImpl) actor).onCreated();
					}
	...
	@SuppressWarnings("unchecked")
	private void setCondition(FxActorImpl actor, Element element) {
		...
		for (Element child : children) {
			...
			if (child.getName().equals("compare")) {
				...
				actor.addCondition(field, method, value, "false".equalsIgnoreCase(result) == false);
	...
	@SuppressWarnings("unchecked")
	private void setParameters(FxActorImpl actor, Element element) {
		List<Element> children = element.getChildren();

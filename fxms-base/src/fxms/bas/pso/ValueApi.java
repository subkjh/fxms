...
import fxms.bas.pso.data.ValMo;
import fxms.bas.pso.exception.PsItemNotFoundException;
...
public abstract class ValueApi extends FxApi {
	...
	/** 수집하고 있는 성능항목 */
	private final Map<String, FX_CF_FXSERVICE_PS> collectedPsCodeMap;
	...
	public ValueApi() {
		...
		collectedPsCodeMap = Collections.synchronizedMap(new HashMap<String, FX_CF_FXSERVICE_PS>());
	}
	...
	public synchronized void addValue(VoList voList, boolean checkAlarm) {
		...
		for (int i = voList.size() - 1; i >= 0; i--) {
			...
			if (ps == null) {
				FX_CF_FXSERVICE_PS e = new FX_CF_FXSERVICE_PS();
	...
	public Map<String, FX_CF_FXSERVICE_PS> getCollectedPsCodeMap() {
	...
	public abstract List<FX_CF_FXSERVICE_PS> doSelectServicePs(String msIpaddr, String serviceName) throws Exception;
	...
	protected void initApi() throws Exception {
		...
		try {
			List<FX_CF_FXSERVICE_PS> list = doSelectServicePs(FxCfg.getCfg().getIpAddress(), FxCfg.getFxServiceName());

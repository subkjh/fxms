...
import fxms.nms.co.snmp.mib.ENTITY_MIB;
import fxms.nms.co.snmp.vo.OidValue;
import fxms.nms.fxactor.conf.snmp.std.CardMgr;
...
public class ConfEntPhysicalActor extends ConfSnmpFxActor {
	 ...
	 */
	private List<NeCardMo> getCardIndexListWithPhysicalClass(NeMo node) throws SnmpTimeoutException, SnmpNotFoundOidException,
			Exception {
	...
	// sensor.setOidValue("");
	// sensor.setMoType(sensor.getMoName().toLowerCase().indexOf("temperature")
	// >= 0
	// ? "TEMP" : "SENSOR");

...
import fxms.nms.dbo.FN_NET_AL_CUR;
import fxms.nms.dbo.FN_NET_AL_HST;
import fxms.nms.dbo.FN_NET_ITEM;
import fxms.nms.vo.NetItemVo;
import fxms.nms.vo.NetVo;
...
public class NetAlarmMaker extends QueueFxThread<Alarm> implements FxServiceMember {
		...
		if (moMap != null && alarmCode != null && alarmCode.isServiceAlarmYn()) {
			...
			if (itemList != null) {
				...
				if (netAlarmMap.size() > 0) {
					DBManager.getMgr().getDataBase(FxCfg.DB_CONFIG).createFxDao()
							.insertOfClass(FN_NET_AL_CUR.class, netAlarmMap.values());
				}
	...
	private void clearNetAlarm(Alarm alarm) {
		...
		try {
			...
			tran.deleteOfClass(FN_NET_AL_CUR.class, null, para);

...
import fxms.bas.po.counter.CounterMgr;
...
import fxms.nms.fxactor.traffic.data.IfOctetMgr;
import fxms.nms.fxactor.traffic.data.IfOctets;
import fxms.nms.fxactor.traffic.data.TrafficValuePort;
...
public class TrafficIfActor extends TrafficFxActor implements NeIfTrafficIndex {
	...
	private List<String> makeSnmpOid(NeMo node, NeIfMo port, TrafficValuePort traffic) throws SnmpTimeoutException, Exception {
	...
	private List<PsVo> makeValue(NeMo node, TrafficValuePort traffic) {
		...
		if (traffic.getInBps() >= 0) {
			...
			// retValue.add(node, NmsCodes.PsItem.NeIfInUsgae,
			// traffic.getInUsage());
	...
	private List<RetryOid> snmpget(NeMo node, List<String> oidList, TrafficValuePort traffic) throws SnmpTimeoutException,
			Exception {

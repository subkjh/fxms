...
import fxms.nms.co.snmp.SnmpUtil.EXIST_RET;
import fxms.nms.co.snmp.mib.HOST_RESOURCES_MIB;
import fxms.nms.co.snmp.mib.HpMIB;
import fxms.nms.co.snmp.vo.OidValue;
import fxms.nms.mo.NeMo;
import fxms.nms.mo.ProcessMo;
		...
		PROC: for (Proc p : procList) {
			for (ProcessMo process : moList) {
				if (process.match(p.name, p.path, p.para)) {
					...
					if (Logger.debug) {
						System.out.println(process.getMoNo() + "," + p.pid + "," + p.name + "," + p.path + "," + p.para);
					}
					continue PROC;
				}
			}
	...
	private List<PsVo> makeValue(long pollMsdate, NeMo node, List<ProcessMo> moList) {
		...
		for (ProcessMo process : moList) {
			...
			if (Logger.logger.isTrace()) {
				Logger.logger.trace(process.toString() + ", pid=" + process.getPidList() + ", run=" + process.getStatusRun()
						+ ", cpu=" + process.getCpuTime() + ", mem=" + process.getMemUsed() + " instance=" + instance);
			}
		...
		if (valueList != null) {
			for (OidValue v : valueList) {
				...
				try {
					...
					res.cpu = CounterMgr.getMgr().getValueCounter(node.getMoNo() + "." + pid + ".cpu", mstime, res.cpu * 10000);
				} catch (Exception e) {
					res.cpu = -1;
				}
				map.put(pid, res);

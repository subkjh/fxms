	...
	public List<PsVo> getValues(long pollMsdate, Mo mo, String... psCodes) throws PollingTimeoutException, Exception {
		...
		if (var.isNull() == false) {
			...
			if (checked == false) {
				...
				EventApi.getApi().checkPsValue(node, null, NmsCodes.PsItem.NeSysUptime, null, node.getNeStatus().getSysUptime(),
						pollMsdate);

				// 1/100초이므로 100을 더 곱합니다.
				if (uptime <= (node.getPollCyclePing() * 100L)) {
					if (FxServiceImpl.fxService != null) {
						FxServiceImpl.fxService.send(new SyncSignal(node.getMoNo(), node.getIpAddress(), "uptime "
								+ node.getNeStatus().getSysUptime(), NeMo.MO_CLASS));
					}
					// return RESYNCED;
				}
